...
if (isset($_SESSION['user'])) {
    ...
    if ($res->num_rows === 1) {
        ...
        $evento = 'Acceso al index.php';
        $fecha = date('Y-m-d H:i:s');

        $stmtLog = $conn->prepare("INSERT INTO logs (ID_USUARIO, IP, EVENTO, FECHA) VALUES (?, ?, ?, ?)");
        $stmtLog->bind_param("isss", $userId, $ip, $evento, $fecha);
        $stmtLog->execute();
    }
}
...
$databases = [];
if ($userId) {
    $stmtDb = $connUser->prepare("SELECT NOMBRE_BD FROM base_datos WHERE ID = ?");
    ...
    <div class="dropdown mt-2">
      ...
      <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <?php foreach ($databases as $db): ?>
          <li>
            <a class="dropdown-item" href="terminal.php?bd=<?= urlencode($db) ?>">
              <?= htmlspecialchars($db) ?>
            </a>
          </li>
        <?php endforeach; ?>
