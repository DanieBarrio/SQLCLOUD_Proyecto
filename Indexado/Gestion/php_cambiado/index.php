...
function getCpuUsagePercent() {
    $load = sys_getloadavg();
    $cpuCount = trim(shell_exec("nproc"));
...
<body>
  ...
  <div class="bg-gray-800 text-white p-6 rounded-lg shadow-lg mt-6">
    ...
    <!-- Botón de crear BD (solo si el plan lo permite) -->
    <?php if ($puedeCrear): ?>
  ...
  <script>
    ...
    document.getElementById('perfilFormModal')?.addEventListener('submit', (e) => {
      e.preventDefault();
      const cambios = Array.from(inputsModal).some(inputElem => inputElem.value.trim() !== valoresOriginalesModal[inputElem.id].trim());
      if (!cambios) {
